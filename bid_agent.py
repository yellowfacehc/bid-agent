...
class BidAgent:
    ...
    def search(
        ...
        """
        all_items: List[BidItem] = []
        # 过滤有效关键词；如果全部为空，则用空字符串表示"搜索全部"
        valid_keywords = [kw.strip() for kw in keywords if kw and kw.strip()]
        if not valid_keywords:
            valid_keywords = [""]  # 空关键词：按日期范围返回所有招投标公告

        # 重置平台状态
        for name in self.platform_status:
            self.platform_status[name] = {"count": 0, "error": "", "ok": False}

        # 多关键词并发搜索: 每个关键词启动一组4平台并行（max_workers至少为1）
        with ThreadPoolExecutor(max_workers=max(min(len(valid_keywords) * 4, 12), 1)) as executor:
            future_to_kw = {}
        ...
        for item in filtered_items:
            ...
            if title_key not in seen:
                ...
                if (item.publish_date or "") > (existing.publish_date or ""):
                    seen[title_key] = item
    ...
    def search_intention(
        ...
        valid_keywords = [kw.strip() for kw in keywords if kw and kw.strip()]
        if not valid_keywords:
            valid_keywords = [""]  # 空关键词：按日期范围返回所有采购意向

        # 【重要】当关键词为空时，只用空关键词搜索，不用区名关键词搜索
        # 原因：区名关键词搜索（如"九龙坡"）只能返回标题含区名的项目（通常只有10条左右），
        #       而很多项目标题不含区名但实际归属目标区县（如重庆开放大学、电力高专、红光中学等）。
        #       空关键词搜索返回所有项目，翻足够多的页后用地理归属过滤，能覆盖更多结果。
        # 当用户输入了自定义关键词时，才用用户的关键词搜索。
        ...
        # 空关键词时增加翻页数量，获取更多历史数据（最多10页）
        effective_max_pages = max_pages
        if valid_keywords == [""]:
            effective_max_pages = max(max_pages, 10)

        # 多关键词并发搜索采购意向（max_workers至少为1）
        with ThreadPoolExecutor(max_workers=max(min(len(valid_keywords) * 2, 6), 1)) as executor:
            future_to_kw = {}
            for kw in valid_keywords:
                logging.info(f"=== 采购意向搜索关键词: {kw} (翻{effective_max_pages}页) ===")
                future = executor.submit(
                    self._search_intention_single, kw, days, effective_max_pages,
                    region, start_date, end_date,
                )
                future_to_kw[future] = kw
    ...
    def _search_intention_single(
        ...
        all_items: List[BidItem] = []
        # 有关键词时抓取详情页做全文匹配，空关键词时不抓以节省时间
        fetch_details = bool(keyword and keyword.strip())

        with ThreadPoolExecutor(max_workers=2) as executor:
            future_to_platform = {}
            for key, crawler in self.intention_crawlers.items():
                if key == "intention_cq":
                    # 重庆采购意向爬虫支持fetch_details参数
                    future = executor.submit(
                        crawler.search, keyword, days, max_pages,
                        region, start_date, end_date, fetch_details,
                    )
                else:
                    # 中国政府采购意向爬虫（暂不支持详情页抓取）
                    future = executor.submit(
                        crawler.search, keyword, days, max_pages,
                        region, start_date, end_date,
                    )
