...
class BidAgent:
    ...
    def search(
        ...
        # 日期过滤 - 仅对有日期的项目做宽松过滤（排除超过1年的过期数据）
        # 注意: 各平台API已自带日期过滤，cqypt(云平台)不支持日期参数，
        # 所以这里不做严格过滤，只排除明显过期（超过365天）的数据
        if start_date and end_date:
            filter_end = datetime.now()
            filter_start = filter_end - timedelta(days=365)
        else:
            filter_end = datetime.now()
            filter_start = filter_end - timedelta(days=365)
        ...
        for item in all_items:
            if item.publish_date:
                try:
                    ...
                    if filter_start <= item_date <= filter_end + timedelta(days=1):
