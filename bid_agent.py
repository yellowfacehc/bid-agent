...
class BidAgent:
    ...
    def search_intention(
        ...
        # 【关键优化】当关键词为空且选择了目标区县时，自动用区名作为补充关键词搜索API
        # 原因：空关键词只返回最近的N条，可能恰好都是市级单位，地理归属过滤后为0条；
        #       用区名搜索可以直接命中标题中包含"九龙坡"/"大渡口"的项目。
        # 同时保留空关键词搜索，两者合并后再做地理归属智能过滤。
        if valid_keywords == [""] and target_districts:
            district_keywords = []
            for d in target_districts:
                # 去掉"区"字，用核心区名搜索（如"九龙坡区"→"九龙坡"）
                core_name = d.replace("区", "") if d.endswith("区") else d
                district_keywords.append(core_name)
            # 保留空关键词搜索 + 区名关键词搜索（合并去重）
            valid_keywords = [""] + district_keywords
            logging.info(f"空关键词+目标区县，自动补充区名搜索: {district_keywords}")
