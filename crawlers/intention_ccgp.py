...
class CCGPIntentionCrawler(BaseCrawler):
    ...
    def _post_via_proxy(self, url: str, data: dict, timeout: int = 15) -> Optional[requests.Response]:
        ...
        # 第二步：尝试通过CORS代理（将POST参数拼接到URL，用GET方式通过代理）
        # 注意：这只在代理支持POST转发时有效，否则返回的可能是列表页而非API结果
        if "网络不可达" in self.last_error or "Connection" in self.last_error:
            logger.info(f"[{self.name}] 直接POST失败，尝试CORS代理中转...")
            try:
                from base import CORS_PROXY_URL
                if CORS_PROXY_URL:
                    # 将POST数据转为URL参数，通过代理GET请求
                    query_string = urlencode(data)
                    full_url = f"{url}?{query_string}"
                    from urllib.parse import quote_plus
                    proxy_url = f"{CORS_PROXY_URL.rstrip('/')}/?url={quote_plus(full_url)}"
                    simple_headers = {
                        "User-Agent": self._get_random_ua(),
                        "Accept": "application/json, text/plain, */*",
                    }
                    resp = requests.get(proxy_url, headers=simple_headers, timeout=25, verify=False)
                    if resp.status_code == 200 and resp.text:
                        logger.info(f"[{self.name}] CORS代理POST成功!")
                        self.last_error = ""
                        return resp
            except Exception as e:
