        ...
        for page in range(1, max_pages + 1):
            ...
            try:
                ...
                for rec in records:
                    ...
                    # 构建详情链接
                    # 优先使用API返回的URL字段，否则按typeCode+id构造
                    item_id = rec.get("id", "")
                    ...
                    if not detail_url and item_id:
                        # 根据typeCode构造URL (格式: /{typeCode}/{id})
                        if type_code:
                            detail_url = f"{BASE_URL}/{type_code}/{item_id}"
                        elif biz_type:
                            detail_url = f"{BASE_URL}/{biz_type}/{item_id}"
                        else:
                            detail_url = f"{BASE_URL}/notice/{item_id}"

                    # 调试日志: 记录构造的URL和可用字段
                    if not detail_url:
                        logger.debug(
                            f"[{self.name}] 无URL字段, 可用字段: "
                            f"id={item_id}, typeCode={type_code}, bizType={biz_type}, "
