...
ORIGIN_URL = "https://www.ccgp-chongqing.gov.cn"
...
class CQYPTCrawler(BaseCrawler):
    ...
    def search(
        ...
        headers = self._get_full_headers(
            referer=f"{BASE_URL}/ge/list?keyword={keyword_encoded}",
            origin=ORIGIN_URL,
            accept_json=True,
        )
        ...
        for page in range(1, max_pages + 1):
            params = {
                ...
                "endTime": str(end_ts),
                "publishStartTime": str(start_ts),
                "publishEndTime": str(end_ts),
            ...
            try:
                ...
                for rec in records:
                    ...
                    # 客户端日期过滤: 检查结果是否在用户指定范围内
                    # 云平台API的日期参数可能不被支持，返回旧数据
