...
<body>
    ...
    <script>
        ...
        function doSearchIntention() {
            ...
            document.getElementById('loadingText').textContent =
                '正在搜索采购意向 ' + keywords.length + ' 个关键词' + districtText + '...（约需15-50秒）';
        ...
        // 切换采购意向模式时显示/隐藏目标区县选择
        document.addEventListener('DOMContentLoaded', function() {
            var intentionBtn = document.getElementById('intentionBtn');
            var intentionSection = document.getElementById('intentionSection');
            var searchModeHint = document.getElementById('searchModeHint');

            // 鼠标悬停采购意向按钮时显示目标区县选择
            intentionBtn.addEventListener('mouseenter', function() {
                intentionSection.style.display = 'block';
                searchModeHint.style.display = 'block';
            });

            // 点击采购意向按钮后保持显示
            intentionBtn.addEventListener('click', function() {
                intentionSection.style.display = 'block';
                searchModeHint.style.display = 'block';
            });
