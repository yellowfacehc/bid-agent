        ...
        function doSearch() {
            ...
            .then(function(r) {
                ...
                return r.text().then(function(text) {
                    if (!text || text.trim() === '') {
                        throw new Error('服务器返回空响应，可能是请求超时（Render免费版限制60秒）或服务未启动，请稍后重试');
                    }
                    try {
                        var data = JSON.parse(text);
                        if (!r.ok) {
                            throw new Error(data.error || '服务器错误 (HTTP ' + r.status + ')');
                        }
                        return data;
                    } catch (e) {
                        if (e instanceof Error && e.message.indexOf('服务器') >= 0) throw e;
                        throw new Error('响应解析失败: ' + e.message + '（可能服务超时）');
                    }
                });
            })
            .then(function(data) {
            ...
            .catch(function(err) {
                ...
                if (msg.indexOf('Failed to fetch') >= 0 || msg.indexOf('NetworkError') >= 0) {
                    msg = '请求超时或网络错误，请稍后重试（搜索可能需要30-60秒）';
                }
                alert('请求失败: ' + msg);
            });
        }
